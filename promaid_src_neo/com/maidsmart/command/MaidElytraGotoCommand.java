...
/**
 * v1.2.2 实测五百八十七【鞘翅赶路 · 手动触发】：{@code /maid_smart elytra_goto <x> <y> <z>}
 * 让最近一只女仆穿鞘翅飞向指定坐标。
 *
 * 【为什么要有这个命令】
 * <ol>
 *   <li>排查用：无头测试服里没有真玩家，就没有"主人"，自动触发（TLM 要瞬移）无从发生，
 *       只能靠这个命令复现整条飞行链路；</li>
...
public final class MaidElytraGotoCommand {
    ...
    private static int gotoPos(CommandSourceStack src, double x, double y, double z) {
        try {
            ServerLevel level = src.getLevel();
            Vec3 pos = new Vec3(x, y, z);
            // 以命令执行点为中心找最近的女仆（与 /maid_smart 其它子命令同口径：64 格）
