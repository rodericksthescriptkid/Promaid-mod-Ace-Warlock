...
public final class BuildPlan {
    ...
    public static final class PlanState {
        ...
        public List<String> toPlan() {
            if (this.cachedPlan == null) {
                ...
                plan.add("O," + origin.getX() + "," + origin.getY() + "," + origin.getZ()
                        + "," + name + "," + planId);
                plan.addAll(steps);
    ...
    public static final class Progress {
         ...
         *  每步都 net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(parse(id))，同种方块大量重复，
    ...
    public static String getBoundPlanId(EntityMaid maid) {
        ...
        if (((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().contains(BOUND_PLAN_TAG, 8)) {
            String saved = ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getString(BOUND_PLAN_TAG);
    ...
    public static String setPlan(net.minecraft.server.level.ServerLevel level, BlockPos origin,
        ...
        PlanState ps = new PlanState(planId, level.dimension(), origin, name, blueprintId,
                new ArrayList<>(steps));
        ...
        LOGGER.info("build plan created: id={} name={} origin={},{},{} planId={}",
                blueprintId, name, origin.getX(), origin.getY(), origin.getZ(), planId);
        return planId;
    }
    ...
    private static BuildArchive.SavedPlan toSavedPlan(PlanState ps) {
        ...
        sp.planId = ps.planId;
        sp.ox = ps.origin.getX();
        sp.oy = ps.origin.getY();
        sp.oz = ps.origin.getZ();
    ...
    public static void sweepOrphans(net.minecraft.server.level.ServerLevel level) {
        ...
        Long last = LAST_SWEEP.get(level.dimension());
        if (last != null && now - last < 300_000) {
            return;
        }
        LAST_SWEEP.put(level.dimension(), now);
    ...
    public static void bindMaid(EntityMaid maid, String planId) {
        ...
        MAID_PLAN.put(maid.getUUID(), planId);
        ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putString(BOUND_PLAN_TAG, planId);
    }

    /** v1.5.180：解绑女仆（离开建筑状态由调用方切任务） */
    public static void unbindMaid(EntityMaid maid) {
        MAID_PLAN.remove(maid.getUUID());
        ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putString(BOUND_PLAN_TAG, "");
    }
    ...
    public static int joinAll(net.minecraft.server.level.ServerLevel level,
        ...
        PlanState ps = PLANS.get(planId);
        if (ps == null || !ps.dim.equals(level.dimension())) {
            return 0;
        }
    ...
    public static void persistCursor(net.minecraft.server.level.ServerLevel level, PlanState ps, int cursor) {
        long now = level.getGameTime(); // getGameTime
    ...
    public static String chooseForeman(net.minecraft.server.level.ServerLevel level, PlanState ps) {
        ...
        return list.get(level.random.nextInt(list.size())).getUUID().toString();
    }
    ...
    public static boolean isForeman(EntityMaid maid) {
        ...
        net.minecraft.server.level.ServerLevel level = maid.level()
        ...
        if (f == null || f.isEmpty()) {
            if (level != null) {
                ...
                if (!nf.isEmpty()) {
                    ...
                    return nf.equals(maid.getUUID().toString());
                }
            }
            return true; // 随机失败（无人可当）→ 放行兜底
        }
        if (f.equals(maid.getUUID().toString())) {
            return true;
        }
        if (level == null) {
            return false;
    ...
    public static boolean isExplicitForeman(EntityMaid maid) {
        ...
        String f = ps.foremanUuid;
        return f != null && !f.isEmpty() && f.equals(maid.getUUID().toString());
    }
    ...
    private static EntityMaid findForemanMaid(net.minecraft.world.level.Level level, String uuid) {
        ...
        for (EntityMaid m : scanAreaMaids(sl)) {
            if (m.getUUID().toString().equals(uuid)) {
                return m;
            }
        }
        return null;
    ...
    public static void ensureChunks(net.minecraft.server.level.ServerLevel level, PlanState ps) {
        ...
        if (box == null) {
            ...
            int minCx = (origin.getX() + minX) >> 4;
            int maxCx = (origin.getX() + maxX) >> 4;
            int minCz = (origin.getZ() + minZ) >> 4;
            int maxCz = (origin.getZ() + maxZ) >> 4;
        ...
        net.minecraft.server.level.TicketType<net.minecraft.world.level.ChunkPos> forced =
                net.minecraft.server.level.TicketType.FORCED; // FORCED
        for (int cx = box[0]; cx <= box[1]; cx++) {
            for (int cz = box[2]; cz <= box[3]; cz++) {
                ...
                level.getChunkSource().addRegionTicket(forced, cp, 0, cp); // addRegionTicket
            }
        }
    ...
    public static void releaseChunks(net.minecraft.server.level.ServerLevel level, PlanState ps) {
        ...
        for (int cx = box[0]; cx <= box[1]; cx++) {
            for (int cz = box[2]; cz <= box[3]; cz++) {
                net.minecraft.world.level.ChunkPos cp = new net.minecraft.world.level.ChunkPos(cx, cz);
                level.getChunkSource().removeRegionTicket(forced, cp, 0, cp); // removeRegionTicket
            }
        }
    }
    ...
    public static PlanState findOverlap(net.minecraft.server.level.ServerLevel level, BlockPos origin,
        ...
        int nx0 = origin.getX() - w / 2;
        int nx1 = nx0 + w;
        int ny0 = origin.getY();
        int ny1 = ny0 + h;
        int nz0 = origin.getZ() - d / 2;
        int nz1 = nz0 + d;
        for (PlanState ps : getPlans(level)) {
    ...
    public static int[] planRegion(PlanState ps) {
        ...
        return new int[]{ps.origin.getX() + minX, ps.origin.getY() + minY, ps.origin.getZ() + minZ,
                ps.origin.getX() + maxX + 1, ps.origin.getY() + maxY + 1, ps.origin.getZ() + maxZ + 1};
    }
    ...
    public static void setMaidPaused(EntityMaid maid, boolean paused) {
        if (paused) {
            MAID_PAUSED.put(maid.getUUID(), true);
        } else {
            MAID_PAUSED.remove(maid.getUUID());
        }
    }

    public static boolean isMaidPaused(EntityMaid maid) {
        return Boolean.TRUE.equals(MAID_PAUSED.get(maid.getUUID()));
    }
    ...
    private static void restoreAll(net.minecraft.server.level.ServerLevel level) {
        ...
        for (BuildArchive.SavedPlan sp : arch.plans) {
            ...
            PlanState ps = new PlanState(sp.planId, level.dimension(),
        ...
        for (int i = 1; i < plan.size(); i++) {
            ...
            try {
                ...
                net.minecraft.core.BlockPos target = new net.minecraft.core.BlockPos(
                        ps.origin.getX() + rx, ps.origin.getY() + ry, ps.origin.getZ() + rz);
                net.minecraft.world.level.block.Block want = net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(
                        net.minecraft.resources.ResourceLocation.parse(pp[3]));
                net.minecraft.world.level.block.state.BlockState st = level.getBlockState(target);
                if (want != null && (st.getBlock() == want
                        || BlueprintLib.isBuiltEquivalent(pp[3], st.getBlock()))) {
