...
public class MaidBuildBehavior extends Behavior<EntityMaid> {
    ...
    protected static void teleportToWorkSite(ServerLevel level, EntityMaid maid, BlockPos target) {
        try {
            ...
            Long last = LAST_TELEPORT.get(maid.getUUID());
            ...
            double distSq = maid.position().distanceTo(
                    new net.minecraft.world.phys.Vec3(stand.getX() + 0.5, stand.getY(), stand.getZ() + 0.5));
            if (distSq < 4.0) {
                return; // 已在该格 2 格内（就在工地旁站着）→ 不瞬移
            }
            maid.setPos(stand.getX() + 0.5, stand.getY(), stand.getZ() + 0.5);
            maid.setDeltaMovement(net.minecraft.world.phys.Vec3.ZERO); // 清速度防瞬移后滑移
            LAST_TELEPORT.put(maid.getUUID(), now);
        } catch (Exception ignored) {
        }
    }
    ...
    private static BlockPos findStandSpot(ServerLevel level, BlockPos target) {
        for (net.minecraft.core.Direction d : net.minecraft.core.Direction.Plane.HORIZONTAL) {
            BlockPos stand = target.relative(d); // relative(Direction)
    ...
    private static boolean standable(ServerLevel level, BlockPos pos) {
        try {
            ...
            net.minecraft.world.level.block.state.BlockState under = level.getBlockState(pos.offset(0, -1, 0));
            return !under.isAir()
                    && !under.getFluidState().is(net.minecraft.tags.FluidTags.WATER);
        } catch (Exception ignored) {
            return false;
        }
    }
     ...
     *  - 处于建造任务 → 若仍是坐姿则站起（setOrderedToSit false，TLM override 连坐姿一起清）；
     ...
     * - 处于建造任务 → 强制 setInSittingPose(true)（setInSittingPose，TLM 原生坐姿）。
     ...
     * 走的是 setOrderedToSit（setOrderedToSit，指令位），TLM 脑内多处判定（含
     * canBrainMoving 与 RandomStroll 挂载的 Await 行为）都认【指令位】，
     * 只按姿势位（setInSittingPose 顺带设了指令位，但玩家"解除坐下"只清指令位、
    ...
    public static void tickBuildSit(EntityMaid maid) {
        ...
        boolean wasSitting = ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().getBoolean(BUILD_SIT_TAG);
        if (building) {
            // v1.1.0 实测二百七十二：建造拟真化——不再强制坐下；若仍坐姿（玩家手动
            // shift+右键/旧版遗留）则站起，站定后由 teleportToWorkSite 瞬移到工地旁放置
            if (maid.isInSittingPose()) { // isInSittingPose → 站起
                maid.setOrderedToSit(false); // setOrderedToSit false（TLM override 连坐姿一起清）
            }
            ...
            net.minecraft.world.phys.Vec3 v = maid.getDeltaMovement();
            maid.setDeltaMovement(new net.minecraft.world.phys.Vec3(0.0, v.y, 0.0));
            if (!wasSitting) {
                ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putBoolean(BUILD_SIT_TAG, true);
            }
        } else if (wasSitting) {
            if (maid.isInSittingPose()) {
                maid.setOrderedToSit(false);
            }
            ((net.neoforged.neoforge.common.extensions.IEntityExtension) maid).getPersistentData().putBoolean(BUILD_SIT_TAG, false);
        }
    }
    ...
    public static void updateActiveBuilders(net.minecraft.server.MinecraftServer server) {
        ...
        for (net.minecraft.server.level.ServerLevel level : server.getAllLevels()) {
            ...
            if (c > 0) {
                ACTIVE_MAIDS.put(level.dimension(), c);
            }
        }
    }
    ...
    public static String lastMissing(EntityMaid maid) {
        return LAST_MISSING.get(maid.getUUID());
    }

    private static void clearMissing(EntityMaid maid) {
        LAST_MISSING.remove(maid.getUUID());
    }
        ...
        if (now - LAST_CANUSE_LOG > 100) {
            ...
            LOGGER.info("build canUse: maid={} buildTask={} task={} plan={} paused={} maidPaused={}",
                    ...
                    maid.getTask() == null ? "null"
                            : maid.getTask().getUid().getPath() + "@" + maid.getTask().getUid().getNamespace(),
    ...
    protected void start(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        this.placeCooldown = maid.getId() % PLACE_INTERVAL;
    ...
    protected void tick(ServerLevel level, EntityMaid maid, long gameTime) {
        ...
        for (; i < size && lookaheadLeft > 0; i++) {
            ...
            Block block = prog.blockCache.computeIfAbsent(blockId,
                    id -> net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(net.minecraft.resources.ResourceLocation.parse(id)));
            if (block == null) {
                if (i == prog.cursor) {
                    prog.cursor = i + 1;
                }
            ...
            if (state.getBlock() == block || BlueprintLib.isBuiltEquivalent(blockId, state.getBlock())
            ...
            if (block == net.minecraft.world.level.block.Blocks.AIR) {
                if (!state.isAir() && BlueprintLib.canBreak(state.getBlock())) {
        ...
        PENDING_RESEND_CHUNKS.add(chunkKeyOf(origin, x, z));
            ...
            if (!state.isAir()) {
                ...
                if (BlueprintLib.canBreak(state.getBlock())) {
        ...
        if (!prog.deferred.isEmpty()) {
            ...
            for (java.util.Iterator<java.util.Map.Entry<Integer, Integer>> it = prog.deferred.entrySet().iterator();
                ...
                if (retryAt != null && retryAt > level.getGameTime()) {
                    reorder.add(idx);
                    continue;
                }
                ...
                if (used != null) {
                    ...
                    if (exactItem != null && used != exactItem) {
                        long nowTick2 = level.getGameTime();
                        Long lastAlt = ALT_NOTIFIED.get(maid.getUUID());
                        if (lastAlt == null || nowTick2 - lastAlt >= 600L) {
                            ALT_NOTIFIED.put(maid.getUUID(), nowTick2);
                            if (BuildPlan.isForeman(maid)) {
                                maid.getChatBubbleManager().addTextChatBubble(
                                        "缺 " + BlueprintLib.cnName(blockId) + "，我用 "
                                                + BlueprintLib.cnName(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(used).toString())
                                                + " 替代一下～");
                            }
                        }
                ...
                if (used == null) {
                    ...
                    if (exactItem == null) {
                        ...
                        prog.deferredRetryAt.put(idx, level.getGameTime() + 40);
                        // v1.5.317：水/岩浆缺的是桶（材料链按桶结算），提示报桶名
                        this.notifyMissing(maid, fluidBucketId(blockId));
                    }
                    continue;
                ...
                if (!doPlace(level, maid, origin, target, plan, placed, machineState, beSnbt,
                    ...
                    if (fails >= 3) {
                        ...
                        try {
                            net.minecraft.world.level.block.state.BlockState ps2 = placed.defaultBlockState();
                            ...
                            if (sup != null) {
                                supPos = target.relative(sup);
                                net.minecraft.world.level.block.state.BlockState supState =
                                        level.getBlockState(supPos);
                                // 仅"支撑格在蓝图内且尚未建（空/流体）"才等蓝图支撑——
                                // 已建但类型不合法（红石线下方玻璃）→ 走 force 补支撑换合法支撑
                                waitPlanSupport = (supState.isAir()
                                        || supState.getFluidState().is(net.minecraft.tags.FluidTags.WATER))
                        ...
                        if (waitPlanSupport) {
                            ...
                            if (supPos != null && !level.isLoaded(supPos)) {
        ...
        if (prog.cursor >= size && prog.deferred.isEmpty()) {
            ...
            this.placeQuota = 0.0;
            if (maid.getOwner() instanceof net.minecraft.server.level.ServerPlayer owner) {
                owner.sendSystemMessage(net.minecraft.network.chat.Component.literal(
                        "\u00a7a【建造完成】" + (maid.getDisplayName() != null ? maid.getDisplayName().getString() : "女仆")
        ...
        if (lastLog == null || level.getGameTime() - lastLog > 400) {
            LAST_STALL_LOG.put(dim, level.getGameTime());
            ...
            for (java.util.Map.Entry<Integer, Integer> e : prog.deferred.entrySet()) {
                ...
                BlockPos t = origin.offset(dx, dy, dz);
                detail.append(" [").append(pp[3].replace("minecraft:", ""))
                        .append('@').append(dx).append(',').append(dy).append(',').append(dz)
                        .append(" 现=").append(net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(level.getBlockState(t).getBlock()))
                        .append(" 载=").append(level.isLoaded(t))
    ...
    private static boolean doPlace(ServerLevel level, EntityMaid maid, BlockPos origin, BlockPos target,
                                   ...
                                   boolean live) {
        BlockState placeState = placed.defaultBlockState();
        ...
        if (sup != null) {
            BlockPos supPos = target.relative(sup); // relative(Direction)
            BlockState supState = level.getBlockState(supPos);
            if (supState.isAir()
                    || supState.getFluidState().is(net.minecraft.tags.FluidTags.WATER)) {
                ...
                if (supState.getFluidState().is(net.minecraft.tags.FluidTags.WATER)
        ...
        // v1.5.82：加 canSurvive 验证（isRedstoneConductor）——支撑不合法（如甘蔗下方是补的
        ...
        boolean bad = targetState.isAir()
                || (targetState.getBlock() != placeState.getBlock()
                    && targetState.getBlock() != placed);
        if (bad) {
            if (fluidSupport) {
                ...
                if (support != null) {
                    ...
                    BlockPos below = target.offset(0, -1, 0);
                    BlockState belowState = level.getBlockState(below);
                    if (belowState.isAir()
                            || belowState.getFluidState().is(net.minecraft.tags.FluidTags.WATER)
                            || force) {
            ...
            if (placed != net.minecraft.world.level.block.Blocks.AIR
                    && (level.getBlockState(target).isAir() || !placeState.isRedstoneConductor(level, target))) {
                logPlaceFail(level, target, placed, "放置后目标仍空气或 canSurvive 失败");
                net.minecraft.world.phys.AABB dropBox = new net.minecraft.world.phys.AABB(target).inflate(1.5);
                for (net.minecraft.world.entity.item.ItemEntity e
                        : level.getEntitiesOfClass(net.minecraft.world.entity.item.ItemEntity.class, dropBox)) {
        ...
        try {
            String placedId = net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(placed.asItem()).toString();
            net.minecraft.world.phys.AABB dropBox2 = new net.minecraft.world.phys.AABB(target).inflate(1.5);
            for (net.minecraft.world.entity.item.ItemEntity e
                    : level.getEntitiesOfClass(net.minecraft.world.entity.item.ItemEntity.class, dropBox2)) {
                net.minecraft.world.item.ItemStack stk = e.getItem();
                if (!stk.isEmpty() && net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stk.getItem()).toString().equals(placedId)) {
                    e.remove(net.minecraft.world.entity.Entity.RemovalReason.DISCARDED);
                }
            }
        } catch (Exception ignored) {
        ...
        if (beSnbt != null) {
            try {
                net.minecraft.nbt.CompoundTag beTag = net.minecraft.nbt.NbtUtils.snbtToStructure(beSnbt);
        ...
        maid.swing(net.minecraft.world.InteractionHand.MAIN_HAND, true);
        // v1.5.60: 放置音效限频 (冲刺模式每秒几百块的音效爆棚防护 - 每 tick 最多 3 个)
        if (lastSoundGameTime != level.getGameTime()) {
            lastSoundGameTime = level.getGameTime();
            soundBudget = 3;
        }
        if (soundBudget > 0) {
            soundBudget--;
            level.playSound(null, target, placeState.getSoundType().getPlaceSound(),
    ...
    private static boolean isAltPlaced(BuildPlan.Progress prog, int idx, BlockState st) {
        ...
        net.minecraft.resources.ResourceLocation key = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(st.getBlock());
    ...
    private static long posKey(BlockPos p, BlockPos origin) {
        return (long) ((p.getX() - origin.getX()) & 0xFFFFF) << 42
                | (long) ((p.getY() - origin.getY()) & 0x1FFFFF) << 21
                | (long) ((p.getZ() - origin.getZ()) & 0x1FFFFF);
    }
    ...
    private static void logPlaceFail(ServerLevel level, BlockPos target, Block placed, String reason) {
        ...
        net.minecraft.resources.ResourceLocation bid = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(placed);
        net.minecraft.resources.ResourceLocation cur = net.minecraft.core.registries.BuiltInRegistries.BLOCK.getKey(level.getBlockState(target).getBlock());
        LOGGER.info("build place-fail: {}@({},{},{}) 原因={} 目标现={}",
                bid, target.getX(), target.getY(), target.getZ(), reason, cur);
    }
    ...
    private static long chunkKeyOf(BlockPos origin, int rx, int rz) {
        int cx = (origin.getX() + rx) >> 4;
        int cz = (origin.getZ() + rz) >> 4;
        ...
        for (Long ck : PENDING_RESEND_CHUNKS) {
            ...
            net.minecraft.network.protocol.game.ClientboundLevelChunkWithLightPacket pkt =
                    new net.minecraft.network.protocol.game.ClientboundLevelChunkWithLightPacket(
                            chunk, level.getChunkSource().getLightEngine(),
    ...
    private static Block planMainBlock(BuildPlan.PlanState ps, List<String> plan) {
        ...
        for (java.util.Map.Entry<String, Integer> e : cnt.entrySet()) {
            ...
            Block b = net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(
        ...
        Block main = best == null ? null : net.minecraft.core.registries.BuiltInRegistries.BLOCK.get(
    ...
    private static boolean scanGaps(ServerLevel level, BlockPos origin, List<String> plan,
        ...
        for (int i = 1; i < plan.size(); i++) {
            ...
            if (st.getBlock() == block || BlueprintLib.isBuiltEquivalent(blockId, st.getBlock())
                    || isAltPlaced(prog, i, st)) {
                continue; // 已建
            }
            if (st.isAir() || BlueprintLib.isAllowedGround(st)) {
    ...
    private void notifyMissing(EntityMaid maid, String blockId) {
        ...
        Long lastCd = MISSING_CD.get(maid.getUUID());
        ...
        MISSING_CD.put(maid.getUUID(), nowTick);
        LAST_MISSING.put(maid.getUUID(), blockId);
    ...
    private static void ensureDoorUpper(ServerLevel level, EntityMaid maid, BlockPos target,
        ...
        for (net.minecraft.world.level.block.state.properties.Property<?> p : state.getProperties()) {
            ...
            try {
                ...
                if (state.getValue(halfProp) != net.minecraft.world.level.block.state.properties.DoubleBlockHalf.LOWER) {
                    return;
                }
